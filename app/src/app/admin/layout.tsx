    ...
    <Link href={item.href}>
      <motion.div
        ...
        className={`relative flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 cursor-pointer group ${
          isActive
            ? 'bg-gradient-to-r from-amber-500/20 to-amber-500/10 text-white border border-amber-500/30'
      ...
      >
        {isActive && (
          <motion.div
            ...
            className="absolute left-0 top-0 bottom-0 w-0.5 bg-gradient-to-b from-amber-400 to-amber-500 rounded-full"
          />
        )}
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (role === "user") {
    return (
      <div className="min-h-screen bg-[#050507] text-white flex items-center justify-center">
        <div className="text-gray-500 text-sm">Admin access required. Redirecting...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#050507] text-white flex">
      ...
      <motion.aside
        ...
        className="fixed inset-y-0 left-0 z-30 w-64 lg:static lg:translate-x-0 flex flex-col bg-[#1A1C22] border-r border-white/[0.06]"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 py-5 border-b border-white/[0.06]">
          ...
          <div>
            <h1 className="text-sm font-bold bg-gradient-to-r from-amber-400 to-amber-400 bg-clip-text text-transparent leading-none">
              PREDICT-X
            </h1>
        ...
        <div className={`mx-4 mt-4 px-3 py-2 rounded-lg border flex items-center gap-2 ${
          ...
          <span className={`w-2 h-2 rounded-full animate-pulse ${statusHealthy ? "bg-green-400" : "bg-red-400"}`} />
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:ml-0">
        {/* Mobile header */}
        <header className="lg:hidden flex items-center gap-4 px-4 py-3 border-b border-white/[0.06] bg-[#1A1C22]/80 backdrop-blur-xl sticky top-0 z-10">
          <button
