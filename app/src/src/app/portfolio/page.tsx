...
function PortfolioPage() {
  ...
  const portfolioStats = (() => {
    ...
    positions.forEach((p) => {
      ...
      const status = getStatusString(p.marketAccount.status);
      if (status === "Settled") {
        settledCount++;
        const yesShares = p.account.yesAmount.toNumber();
        const noShares = p.account.noAmount.toNumber();
        const won =
          (p.marketAccount.winningOutcome.yes && yesShares > 0) ||
          (p.marketAccount.winningOutcome.no && noShares > 0);
        if (won) winsCount++;
      }
    });
    ...
    return {
      ...
      winRate: settledCount > 0 ? Math.round((winsCount / settledCount) * 100) : null,
    };
  })();
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      ...
      {/* Portfolio Overview */}
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-3xl">
